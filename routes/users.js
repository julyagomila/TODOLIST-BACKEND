...
router.post("/user/signup", async (req, res) => {
  try {
    //On vérifie que le username a bien été rentré
    if (req.fields.username === undefined) {
      res.status(400).json({ message: "Il manque un paramètre" });
    } else {
      //On vérifie que l'email en base de données est présent
      const isUserExist = await User.findOne({ email: req.fields.email });
      if (isUserExist !== null) {
        res.json({ message: "Cet email est déjà lié à un compte !" });
      } else {
...
router.post("/user/login", async (req, res) => {
  try {
    ...
    if (userToCheck === null) {
      res.status(401).json({ message: "Vous devez créer un compte" });
    } else {
